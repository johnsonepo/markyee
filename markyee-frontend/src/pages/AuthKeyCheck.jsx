 ...
 function AuthKeyCheck() {
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            try {
                const response = await axios.get(`https://markyee-server.envoos.com/markyee-backend/api/user/auth/${auth_key}`);
                ...
                setErrors({ error: 'Error fetching user details' });
                setLoading(false);
            }
        };
    ...
    const activateUserRequest = async (e) => {
        ...
        try {
            ...
            console.log(authToken ,inputValue, userData.auth_key );
            if (userData.auth_key !== inputValue) {
                setErrors({ error: 'Invalid Auth Key' });
            } else {
                const response = await axios.post(`https://markyee-server.envoos.com/markyee-backend/api/user/detail/${userData.auth_key }`);
                if (response.data.is_verified === 1) {
                    navigate('/signin');
                } else {
